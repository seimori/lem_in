...
void		ft_catline(char **line, unsigned int *count, char *buf)
{
	char	*tmp;
...
int			ft_hasnewline(char *line, char *source, char *buffer)
{
	char	*tmp;
	size_t	len;
...
int			ft_fill_line(char **line, t_buffer *buffer)
{
	...
	while ((ret = read(buffer->fd, buf, BUFF_SIZE)))
	{
		...
		if ((ft_hasnewline(*line, buf, buffer->str)))
			...
			ft_catline(line, &count, buf);
	}
	if ((ft_hasnewline(*line, buffer->str, buffer->str)))
		return (1);
	else
...
int			get_next_line(const int fd, char **line)
{
	...
	*line = ft_memalloc(LINE_SIZE + 1);
	...
	return (ft_fill_line(line, buffer));
}
